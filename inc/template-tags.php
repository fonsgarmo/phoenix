...
if ( ! function_exists( 'phoenix_posted_on' ) ) :
	...
	function phoenix_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'phoenix' ),
...
if ( ! function_exists( 'phoenix_posted_by' ) ) :
	...
	function phoenix_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'phoenix' ),
	...
	function phoenix_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'phoenix' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'phoenix' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'phoenix' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
